...
class ReservationsRepository {
    ...
    getTeamInfoSix = async(arr)=> {
        const matchInfoSix = await Promise.all(arr.map(async(val)=> {
            let teamsData = await Teams.findOne({ where : { teamName : val.teamName}});
            let placesData = await Places.findOne({ where : { spotName : val.place }});
            return {teamsData, placesData};
        }));
        // console.log("--------------", matchInfoSix[1])
        return matchInfoSix;
    };
